...
import { SendHorizonal, SendToBack, Smile } from 'lucide-react'

function Messages() {
  return (
    <div className="relative flex flex-col flex-1">
    <div className="z-20 flex flex-grow-0 flex-shrink-0 w-full pr-3 bg-white border-b">
      <div className="w-12 h-12 mx-4 my-2 bg-blue-500 bg-center bg-no-repeat bg-cover rounded-full cursor-pointer" style={{ backgroundImage: 'url(https://images.unsplash.com/photo-1529626455594-4ff0802cfb7e?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=200&q=50)' }}></div>
      <div className="flex flex-col justify-center flex-1 overflow-hidden cursor-pointer">
        <div className="overflow-hidden text-sm font-medium leading-tight text-gray-600 whitespace-no-wrap">Karen</div>
        <div className="overflow-hidden text-xs text-green-600  leading-tight text-gray-600 whitespace-no-wrap">Online</div>
      </div>
      ...
      <div className="relative flex items-center self-center w-full max-w-xl p-4 overflow-hidden text-gray-600 focus-within:text-gray-400">
        <div className="w-full">
          <span className="absolute inset-y-0 left-0 flex items-center pl-6">
            ...
            <button type="submit" className="p-1 focus:outline-none focus:shadow-none hover:text-green-600">
             <SendHorizonal size={18} color='green'/>
            </button>
          </span>
            <input type="text" className="w-full items-center h-10 pl-10 pr-4  bg-white  text-xs border border-gray-300 rounded-md focus:border-gray-200 focus:outline-none focus:ring-1 focus:ring-offset-0 focus:ring-green-600 transition-colors duration-300" placeholder="Type your message..." />
        </div>
      </div>
    </div>
  </div>
